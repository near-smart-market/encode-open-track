...
async function execute() {
    const options = cla([
        ...
        {
            name: "customerAccountId", alias: "i", type: String
        }
    ]);
...
async function initContract(contractName: string, account: Account) {
    return new Contract(
        ...
        contractName,
        {
            ...
            viewMethods: ["list_customer_orders"], // view methods do not change state but usually return a value
...
async function run(options: cla.CommandLineOptions) {
    ...
    const response = await contract.list_customer_orders({ account_id: options.accountName, customer_account_id: options.customerAccountId });
