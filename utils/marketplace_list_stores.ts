...
async function initContract(contractName: string, account: Account) {
    return new Contract(
        ...
        contractName,
        {
            ...
            viewMethods: ["list_stores"], // view methods do not change state but usually return a value
...
async function run(options: cla.CommandLineOptions) {
    ...
    const response = await contract.list_stores({ account_id: options.accountName });
